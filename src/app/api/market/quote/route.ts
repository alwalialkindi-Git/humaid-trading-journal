...
import { fetchQuote } from "@/lib/market-data/service";
import { ProviderError } from "@/lib/market-data/types";

/** GET /api/market/quote?symbol=EMAAR.AE[&exchange=DFM] — auth required. */
export async function GET(request: Request) {
  ...
  try {
    const response = await fetchQuote(symbol, exchange);
